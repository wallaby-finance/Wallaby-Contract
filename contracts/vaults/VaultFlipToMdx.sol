...
contract VaultFlipToMdx is VaultController, IStrategy, RewardsDistributionRecipientUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /* ========== CONSTANTS ============= */

    address private constant mdx = 0x361B50a630b978eBC6BB4a99163625D5D34fC13d;
    IMasterChef private constant mdx_MASTER_CHEF = IMasterChef(0xb7E089CeA150ff0fb5a4c92E829e129B56A29cD3);
    PoolConstant.PoolTypes public constant override poolType = PoolConstant.PoolTypes.FlipTomdx;
    ...
    function initialize(uint _pid) external initializer {
        ...
        _stakingToken.safeApprove(address(mdx_MASTER_CHEF), uint(~0));
        ...
        setMinter(IWallabyMinter(0xb31A420dD543B9CFb346aF6fFdaA9df5c5BB6c4C));
    ...
    function _deposit(uint amount, address _to) private nonReentrant notPaused updateReward(_to) {
        require(amount > 0, "VaultFlipTomdx: amount must be greater than zero");
        _totalSupply = _totalSupply.add(amount);
        _balances[_to] = _balances[_to].add(amount);
        _depositedAt[_to] = block.timestamp;
        _stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        mdx_MASTER_CHEF.deposit(pid, amount);
    ...
    function withdraw(uint amount) override public nonReentrant updateReward(msg.sender) {
        ...
        mdx_MASTER_CHEF.withdraw(pid, amount);
        uint withdrawalFee;
        if (canMint()) {
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            if (canMint()) {
                performanceFee = _minter.performanceFee(mdxBalance);
                _minter.mintFor(mdx, 0, performanceFee, msg.sender, _depositedAt[msg.sender]);
            }

            IERC20(mdx).safeTransfer(msg.sender, mdxBalance.sub(performanceFee));
            emit ProfitPaid(msg.sender, mdxBalance, performanceFee);
        }
    }

    function harvest() public override {
        mdx_MASTER_CHEF.withdraw(pid, 0);
        _harvest();
    }
    ...
    function _harvest() private {
        ...
        _rewardsToken.deposit(mdxAmount);
    ...
    function setMinter(IWallabyMinter _minter) override public onlyOwner {
    ...
    function setRewardsToken(address newRewardsToken) public onlyOwner {
        require(address(_rewardsToken) == address(0), "VaultFlipTomdx: rewards token already set");

        _rewardsToken = IStrategy(newRewardsToken);
        IERC20(mdx).safeApprove(newRewardsToken, 0);
        IERC20(mdx).safeApprove(newRewardsToken, uint(~0));
    }
    ...
    function _notifyRewardAmount(uint reward) private updateReward(address(0)) {
        ...
        require(rewardRate <= _balance.div(rewardsDuration), "VaultFlipTomdx: reward rate must be in the right range");
    ...
    function setRewardsDuration(uint _rewardsDuration) external onlyOwner {
        require(periodFinish == 0 || block.timestamp > periodFinish, "VaultFlipTomdx: reward duration can only be updated after the period ends");
    ...
    function recoverToken(address tokenAddress, uint tokenAmount) external override onlyOwner {
        require(tokenAddress != address(_stakingToken) && tokenAddress != _rewardsToken.stakingToken(), "VaultFlipTomdx: cannot recover underlying token");
